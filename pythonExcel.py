...
class MedicalAppV3:
    def __init__(self, root):
        self.root = root
        self.root.title("医疗费报销系统 V3.7 (退出确认+只读路径版)")
    ...
    def on_closing(self):
        """退出程序前的确认弹窗"""
        if messagebox.askokcancel("退出确认", "确定要退出医疗费报销系统吗？\n未生成的二维码数据将会丢失。"):
            self.root.destroy()
    ...
    def setup_ui(self):
        ...
        # --- 核心修改：输入框设为 readonly，禁止手动输入 ---
        ent_path = tk.Entry(manage_frame, textvariable=self.base_dir, width=45, state='readonly', fg="gray")
    ...
    def create_new_serial(self):
        ...
        if not os.path.exists(base):
            messagebox.showerror("路径错误", "当前的存档根目录无效，请重新选择路径。")
            return
